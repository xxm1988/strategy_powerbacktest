...
class BacktestEngine:
    ...
    def _fetch_data_with_warmup(self, symbol: str, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """Fetch data including warmup period"""
        if hasattr(self.strategy, '_warmup_period'):
            # Calculate extended start date based on trading days
            calendar = self.data_fetcher.get_trading_calendar(start_date, end_date)
            
            # Calculate required warmup days based on timeframe
            periods_per_day = {
                '1m': 390,  # Trading minutes per day
                '5m': 78,
                '15m': 26,
                '30m': 13,
                '1h': 6.5,
            ...
            periods_per_day_value = periods_per_day.get(self.timeframe, 390)
            warmup_days = max(1, self.strategy._warmup_period // periods_per_day_value)
            
            # Find extended start date
            if len(calendar[calendar <= start_date]) >= warmup_days:
                extended_start = calendar[calendar <= start_date].iloc[-warmup_days]
            else:
                extended_start = calendar.iloc[0]
                self.logger.warning(
                    f"Insufficient calendar days for warmup. Using earliest available date: {extended_start}"
                )
            # Fetch extended data
            data = self.data_fetcher.fetch_historical_data(
                symbol, extended_start, end_date, self.timeframe
            )
            
            # Mark actual backtest period
            data['in_backtest_period'] = data['time_key'] >= pd.Timestamp(start_date)
            return data
        else:
