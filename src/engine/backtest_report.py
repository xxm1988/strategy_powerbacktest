...
class BacktestReport:
    ...
    def _generate_html_report(self, output_dir: str) -> None:
        ...
        template_path = os.path.join(
            os.path.dirname(__file__), "../templates/backtest_report.html"
        )
        ...
        # Prepare chart data
        equity_curve_data = [
            [int(t.timestamp() * 1000), v]
            for t, v in zip(self.portfolio.index, self.portfolio["total"])
        ]

        # Prepare price chart data
        price_data = [
            [int(t.timestamp() * 1000), float(p)]
            for t, p in zip(self.portfolio.index, self.portfolio["close"])
        ]
        ...
        drawdown_data = [
            [int(t.timestamp() * 1000), d * 100]
            for t, d in zip(self.portfolio.index, self._calculate_drawdowns())
        ]

        # Format monthly returns data for Highcharts
        monthly_returns_data = [
            [int(t.timestamp() * 1000), float(r * 100)]
