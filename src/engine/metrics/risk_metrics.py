...
class RiskMetrics:
    ...
    @staticmethod
    def calculate_sortino_ratio(portfolio: pd.DataFrame, risk_free_rate: float = 0.02) -> float:
    ...
    @staticmethod
    def calculate_value_at_risk(portfolio: pd.DataFrame, confidence: float = 0.95) -> float:
        """Calculate Value at Risk"""
        returns = portfolio['returns']
        return np.percentile(returns, (1 - confidence) * 100)

    @staticmethod
    def calculate_beta(portfolio: pd.DataFrame, market_returns: Optional[pd.Series] = None) -> float:
