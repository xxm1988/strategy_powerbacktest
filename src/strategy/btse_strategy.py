...
class BTSEStrategy(BaseStrategy):
    ...
    def calculate_strength_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        str4 = pd.Series(np.where((str2 > 0) & (str2 > str3), str2, 0)).rolling(self.lookback_period).sum()
        
        # STR5:=SUM(IF(STR3>0 AND STR3>STR2,STR3,0),25)
        str5 = pd.Series(np.where((str3 > 0) & (str3 > str2), str3, 0)).rolling(self.lookback_period).sum()
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        condition_a = (strength['str7'] > strength['str6']) & (strength['str7'] > 25) & (strength['str6'] < 25)
