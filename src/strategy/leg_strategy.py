...
class LEGStrategy(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        xx01 = talib.SUM(amv0, timeperiod=self.vol_period) / talib.SUM(data['volume'], timeperiod=self.vol_period)
        
        # Stochastic
        rsv = (close - talib.MIN(low, timeperiod=9)) / (talib.MAX(high, timeperiod=9) - talib.MIN(low, timeperiod=9)) * 100
