...
def test_empty_dataframe():
    ...
    empty_df = pd.DataFrame(columns=['time_key', 'open', 'high', 'low', 'close', 'volume'])
...
def test_non_trading_hours_handling():
    """Test handling of non-trading hours data"""
    # Create data with gaps (non-trading hours)
    dates = []
    for day in range(3):
        base_date = datetime(2023, 1, 1 + day, 9, 30)
        dates.extend([base_date + timedelta(minutes=i) for i in range(390)])
